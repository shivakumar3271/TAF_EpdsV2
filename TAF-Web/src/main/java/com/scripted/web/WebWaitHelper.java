...
public class WebWaitHelper {
				...
				.until(new Function<WebDriver, Boolean>() {
					...
					public Boolean apply(WebDriver d) {
						try {
							...
							/*
							 * Assert.
							 * fail("Error occurred while performing  waitForClear action for  locator :" +
							 * webEle + "Exception :" + e);
							 */
						}
						return canclick;
	...
	public static void waitForPresence(By byEle, int time) {
		try {
			...
			WebDriverWait wait = new WebDriverWait(BrowserDriver.getDriver(), time, 120000);
